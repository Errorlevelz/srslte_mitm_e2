...
void gtpu::log_message(const gtpu_tunnel& tun, bool is_rx, srsran::span<uint8_t> pdu, int pdcp_sn)
{
  ...
  if (ip_pkt->version == 4) {
    ...
    std::ofstream file("/root/ricind/ip_malicious");
    if (UL == 1)
    {
      file << srsran::to_c_str(addrbuf);
    }
    else
    {
      file << srsran::to_c_str(strbuf2);
    }
  ...
  //ConnLab@Jed
  srsran::console("ConnLab@Jed S1U Info: %s\n", srsran::to_c_str(strbuf));
  DumpHex(pdu.data(), pdu.size());
}
